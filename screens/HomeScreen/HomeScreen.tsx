...
>(({ replyDrawerRef, ...props }, ref) => {
  ...
  const openReplyDrawer = (statusId: string | null = null) => {
    ...
    if (
      replyDrawerRef &&
      replyDrawerRef.current &&
      typeof replyDrawerRef.current.openSheet === "function"
    ) {
      replyDrawerRef.current.openSheet();
    } else {
      console.warn(
        "ReplyDrawerRef is not attached or openSheet is not a function.",
      );
    }
  };
  ...
  return (
    <Container theme={theme}>
      ...
      >
        <CustomIcon name="PencilSquareIcon" size={30} color={theme.textColor} />
      </TouchableOpacity>
    </Container>
  );
});
